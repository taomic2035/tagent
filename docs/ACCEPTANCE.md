...
## Step 15 真机验收报告（AC16-1~4，2026-09-01，完成谓词与引导包）

> 来源：SURVEY 第二梯队。完成谓词 = clowder "最深的一条"（LLM 自证不算数，
> 完成由机器可检证据裁决）；引导包 = clowder F24。skills 渐进披露明确不做
> （YAGNI：当前 5 工具/数行 system prompt，收益低于成本，REQUIREMENTS §20）。

| # | 判定 | 证据 |
|---|---|---|
| AC16-1 谓词单测 | ✅ | 6 项正反例：未调用/参数不符/结果失败/无终答各自裁决失败且证据指明缺口（core 99+41=140 项全绿） |
| AC16-2 机器裁决 | ✅ | verify-acceptance.mjs 对 win-ac-1~5 全过（证据含调用 id/参数/结果信封/终答片段）；场景 6（/dump）如实注记为 CLI 命令验证非任务 |
| AC16-3 引导包 | ✅ | /save t15 → /load t15 恢复（5 条+引导包）→ 续问 1 轮完成；transcript 实证恢复后首请求含引导消息 |
| AC16-4 回归 | ✅ | 六场景 stdout 验收 + 机器裁决双双全过 |

### 验收中逼出的既有 bug（第 5 例）

`handleCommand` 的 `switch(line)` 全等匹配——**带参数命令（/save 名字、/load 名字）
从未进过对应 case**，一直落"未知命令"。AC16-3 真机测试首跑抓出，修为按首 token
匹配。Step 6 当年验收走的裸命令形态，掩盖了此缺陷四步之久。

### 设计取舍记录

- 谓词证据必须引用具体事件（调用 id/结果摘要/终答片段）——裁决可复核，
  不是布尔黑箱
- 场景 5（火星）谓词是 `finalIncludes:"无法"`——诚实说明查不了就是该场景的
