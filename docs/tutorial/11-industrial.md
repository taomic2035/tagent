# 第 11 章 参考阅读：三个真实 agent 的独特机制

> **本章是阅读指南，不是教程**——告诉你哪些模式存在于哪些项目里、为什么
> 重要，但不带你实现。**实现是第 12~15 章的事**（第三册：从能跑到敢用）。
>
> 建议用法：先通读本章建立全景认知 → 进入第 12~15 章动手实现 → 实现完
> 回来对照"你做的和工业版差在哪"。全部引用可在本地源码复核（见
> docs/SURVEY-DEEP.md 的路径索引）。
