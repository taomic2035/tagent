...
"agent 自进化"不是一个机制，是四个决策点的组合——**写入决策**（这条经验
值得记吗、谁判断）→**存储形态** →**召回决策**（下个任务该想起哪些）→
**淘汰机制**。任何一环缺席，记忆库就变成只涨不读的垃圾堆。

**① "有没有用"谁判断——三条路线**：

- **模型自判**（本教程路线）：remember 做成工具，模型按语义理解自决；
  工业版是 hermes 的 background_review——每轮结束后**后台 fork 一个自己**
  自审"这轮该存什么"，不占主对话注意力
- **权力分界**（clowder 最深的一课）：探测器只报事实、**永不判重要性**；
  agent 显式三选一 propose / defer / abstain。"是否有用"是价值判断，
  埋进自动逻辑会被无关经历灌满、且失去拒绝 prompt 注入的最后机会
- **事后验证**：一条记忆"有用"的唯一铁证是**后来被用上**。hermes 的
  curator 拿 use_count 统计，但语义讲究："use_count=0 是**证据缺失**，
  不是过时证据"——所以永不删除只归档（30 天标 stale、90 天归档、可恢复）

**② 存什么**：事实（模型"知道"）之上还有 skill（模型"会做"）——把某类
任务的操作手册沉淀成 SKILL.md，是程序性知识的进化，比记偏好更接近"变强"。

**③ 读什么**：本教程 6.6 的 A/B（工具召回 > 静态注入 > 逐问注入）+ clowder
的克制路线（召回也做封闭目录的 cue，禁止开放式 RAG 硬塞）。

**④ 天花板（最值钱的一条）**：推理层自进化有硬上限——**放进上下文的
"经验"本质还是 prompt**，每次重付 token、不改模型参数。真正的跃迁只有
两条路：能写成 skill/代码的绝不写成记忆；再往上走训练层闭环（hermes 把
agent 轨迹压成 SFT 数据喂下一代模型——自进化的终点是进化模型，不是进化 prompt）。

## 深入一层 ⑤：重要性分级与 agent 级 KV 命中率清单

**信息价值分级**（取舍的依据——重要性不是感觉，是层级）：

| 级 | 内容 | 处置 |
|---|---|---|
| 1（钉子） | user 指令原文 | 永不改写、永不丢弃（6.4 的裁决） |
| 2 | 工具结果（数据事实） | 可降级（压成一行保骨架）、可摘要——信息在结构里 |
| 3 | assistant 终答 | 可摘要；旧轮可丢 |
| 4 | 思考过程（reasoning） | **第一优先丢弃**——过程性中间产物，结论已提炼 |
| 5 | 寒暄/客套 | 第一批扔 |

工具结果比模型的话值钱（数据 vs 观点）、思考比正文廉价（过程 vs 结论）——
这是双水位裁剪"从最旧工作单元丢起"背后的价值序。

**agent 级 KV 缓存命中率清单**（每条都改"渲染后文本的字节前缀"）：

1. **前缀只增不改**：steering/降级注入一律 append，不重排不重写（第 7 章）
2. **一次裁到位**：双水位批量手术，两次触发之间前缀逐字节稳定
   （78% 命中 vs "每轮删一点"归零——6.3②的实测表）
3. **system prompt 静态在头**：大静态骨架 + 小易变尾巴（hermes 的前缀
   注册表：把骨架的字节边界登记给缓存规划器）——时间戳/每轮变动的计数器
   **永远不要**放进前段
4. **工具列表顺序稳定**：说明书按注册顺序渲染进文本——新增/重排工具 =
   整段前缀失效。注册顺序应当确定（排序后注册），工具增删接受一次全量
   失效的代价
6.5. **易变数据放消息尾部**：当前轮的动态信息追加在后段，前段放历史与规则
6. **引擎亲和**：llama.cpp 的 KV cache 按 slot 隔离——同会话请求路由到同一
   slot（默认行为即如此），换 slot = 冷缓存；观察窗口就是 `cache_n`
7. **会话级取舍**（clowder 的账）：超长会话"压缩续命"（每轮全量失效一次）
   还是"封存 handoff 开新会话"（旧缓存自然过期）——按会话长度算账，
   没有万能答案

判定口径：**命中率看 cache_n，策略看前缀字节**。一切优化最终都落到
"渲染后文本的前缀逐字节不变"这一句话上。
